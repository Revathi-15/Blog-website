...
let weatherBody = document.getElementById('weather-body'); // Selecting the weather body
weatherBody.style.display = 'none'; // Initially hide the weather body
...
function getWeatherReport(city) {
    if (city.trim() === "") {
        swal("Enter Input", "Please enter a city name", "warning"); // Show alert if input is empty
        weatherBody.style.display = 'none'; // Hide the weather body
        return; // Prevent making the API request
    }
...
function showWeaterReport(weather) {
    ...
    if (city_code === '400') {
        ...
        swal("Enter Valid Input", "Warning..!!");
        reset();
    } else {
        weatherBody.style.display = 'block'; // Show weather body when input is valid
